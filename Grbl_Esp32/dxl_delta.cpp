...
const float re =  133.5; // radius of end effector side (length of linkages) - Tapster 3
//const float re =  223.5; // radius of end effector side (length of linkages) - Tapster 3+
...
uint8_t my_servo_ids[N_AXIS] = {DXL_MOTOR_0_ID, DXL_MOTOR_1_ID, DXL_MOTOR_2_ID, DXL_MOTOR_3_ID, DXL_MOTOR_4_ID, DXL_MOTOR_5_ID};
...
void machine_init() {
  ...
  */
  ...
  Pin1_UserIoControl.set_mode(USER_MODE_PWM_LOW_HIGH);
  Pin1_UserIoControl.set_pwm_freq_bits(50, 16);
  Pin1_UserIoControl.set_pwm_low_high(settings.machine_int16[0], (uint16_t)settings.machine_float[0]);  // $80, $90
  Pin1_UserIoControl.init(); // reinit because PWM params changed

  Pin2_UserIoControl.set_mode(USER_MODE_PWM_LOW_HIGH);
  Pin2_UserIoControl.set_pwm_freq_bits(50, 16);
  Pin2_UserIoControl.set_pwm_low_high(settings.machine_int16[1], (uint16_t)settings.machine_float[1]);  // $81, $91
  Pin2_UserIoControl.init(); // reinit because PWM params changed

  Pin3_UserIoControl.set_mode(USER_MODE_PWM_LOW_HIGH);
  Pin3_UserIoControl.set_pwm_freq_bits(50, 16);
  Pin3_UserIoControl.set_pwm_low_high(settings.machine_int16[2], (uint16_t)settings.machine_float[2]); // $82, $92
  Pin3_UserIoControl.init(); // reinit because PWM params changed

  Pin4_UserIoControl.set_mode(USER_IO_MODE_SPIKE_HOLD_OFF);
  Pin4_UserIoControl.set_spike_length((uint8_t)settings.machine_int16[3]);   // $83
  Pin4_UserIoControl.set_spike_hold_perecent(100, (uint8_t)settings.machine_float[3]); //  $93
  ...
  dxl_ping(DXL_MOTOR_3_ID);
  dxl_ping(DXL_MOTOR_4_ID);
  dxl_ping(DXL_MOTOR_5_ID);
  ...
  dxl_operating_mode(DXL_MOTOR_3_ID, DXL_CONTROL_MODE_POSITION);
  dxl_operating_mode(DXL_MOTOR_4_ID, DXL_CONTROL_MODE_POSITION);
  dxl_operating_mode(DXL_MOTOR_5_ID, DXL_CONTROL_MODE_POSITION);

  // get the current position
  dxl_read_position();

  //dxl_goal_position((int32_t)DXL_MOTOR_3_ID, (int32_t)2000);
  //dxl_goal_position((int32_t)DXL_MOTOR_4_ID, (int32_t)2000);
  //dxl_goal_position((int32_t)DXL_MOTOR_5_ID, (int32_t)2000);
  ...
  if (stepper_idle) {
    ...
    if (moved) {
      ...
      if (status != KIN_ANGLE_CALC_OK) {
        ...
        my_pos[3] = current_position[A_AXIS];
        my_pos[4] = current_position[B_AXIS];
        my_pos[5] = current_position[C_AXIS];
        //grbl_msg_sendf(CLIENT_SERIAL, MSG_LEVEL_INFO,"DXL Move: %d, %d, %d, %d, %d, %d",
        //                              my_pos[0], my_pos[1], my_pos[2],
        //                              my_pos[3], my_pos[4], my_pos[5]);
...
void dxl_read_position() {
  uint32_t encoder[6]; // actual values read from servo in encoder counts 0-4095
  float theta[3];      // calculated degrees from encoder count
  float x, y, z, a, b, c;    // calculated xyz position from servo angles
  ...
  encoder[3] = dxl_present_position(DXL_MOTOR_3_ID);
  encoder[4] = dxl_present_position(DXL_MOTOR_4_ID);
  encoder[5] = dxl_present_position(DXL_MOTOR_5_ID);
  ...
  // No forward kinematics for side button acutators (Axis A, B, and C)
  a = (float)encoder[3];
  b = (float)encoder[4];
  c = (float)encoder[5];
  ...
  sys_position[A_AXIS] = a * settings.steps_per_mm[A_AXIS];
  sys_position[B_AXIS] = b * settings.steps_per_mm[B_AXIS];
  sys_position[C_AXIS] = c * settings.steps_per_mm[C_AXIS];
...
void dxl_delta_torque_enable(bool enable)
{
  ...
  dxl_torque_enable(DXL_MOTOR_3_ID, enable);
  dxl_torque_enable(DXL_MOTOR_4_ID, enable);
  dxl_torque_enable(DXL_MOTOR_5_ID, enable);
...
void forward_kinematics(float *position)
{
  ...
  position[A_AXIS] += gc_state.coord_system[A_AXIS]+gc_state.coord_offset[A_AXIS];
  position[B_AXIS] += gc_state.coord_system[B_AXIS]+gc_state.coord_offset[B_AXIS];
  position[C_AXIS] += gc_state.coord_system[C_AXIS]+gc_state.coord_offset[C_AXIS];
  ...
  calc_fwd[A_AXIS] = position[A_AXIS];
  position[A_AXIS] = calc_fwd[A_AXIS] - (gc_state.coord_system[A_AXIS]+gc_state.coord_offset[A_AXIS]);

  calc_fwd[B_AXIS] = position[B_AXIS];
  position[B_AXIS] = calc_fwd[B_AXIS] - (gc_state.coord_system[B_AXIS]+gc_state.coord_offset[B_AXIS]);

  calc_fwd[C_AXIS] = position[C_AXIS];
  position[C_AXIS] = calc_fwd[C_AXIS] - (gc_state.coord_system[C_AXIS]+gc_state.coord_offset[C_AXIS]);
}
